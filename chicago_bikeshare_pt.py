...
for index in range(21):
    print('Linha {}, gênero: {}'.format(index, data_list[index][-2]))
...
male = len([a for a in column_to_list(data_list, -2) if a == 'Male'])
female = len([a for a in column_to_list(data_list, -2) if a == 'Female'])
...
print('Criando e testando função count_type; Customer, dependent, Subscriber')


def count_type(data_list):
    customer = len([a for a in column_to_list(
        data_list, -3) if a == 'Customer'])
    dependent = len([a for a in column_to_list(
        data_list, -3) if a == 'Dependent'])
    subscriber = len([a for a in column_to_list(
        data_list, -3) if a == 'Subscriber'])
    return [customer, dependent, subscriber]


print(count_type(data_list))
# criando o gráfico de user_type
...
answer = "Alguns espaços na coluna de gender estão vazios"
...
# transform everyline in trip from string to int
